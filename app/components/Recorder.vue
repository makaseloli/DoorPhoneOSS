...
<template>
    <div class="space-y-4">
        <UAlert description="古い録音は削除されます。" variant="outline" color="warning" icon="ic:outline-warning-amber" />
        <UAlert description="仕組み上必ずしも録音が届くとは限りません。重要なメッセージは別の方法で伝えてください。" variant="outline" color="error"
            icon="ic:outline-error-outline" />
        <div class="flex flex-wrap items-center gap-3">
            <UButton v-if="!isRecording" icon="ic:outline-mic-none" color="primary" :disabled="isRecording"
                @click="startRecording">
                録音開始
            </UButton>
            <UButton v-if="isRecording" icon="ic:outline-stop-circle" color="error" :disabled="!isRecording"
                @click="stopRecording">
                録音停止
            </UButton>
            <UButton v-if="isRecording" icon="ic:outline-refresh" variant="outline"
                :disabled="!recordedBlob && !isRecording" @click="resetRecording">
                やり直す
            </UButton>
        </div>
        ...
        <div v-if="audioUrl" class="space-y-3">
            ...
            <div class="flex flex-wrap gap-3">
                <UButton icon="ic:outline-send" color="success" :loading="isSending" @click="sendRecording">
                    音声を送信
                </UButton>
                <UButton icon="ic:outline-close" color="error" :disabled="isSending" @click="resetRecording">
                    破棄する
                </UButton>
            </div>
        </div>
